    ...
    return (
      <div data-theme={theme} className="min-h-screen bg-gradient-to-br from-slate-950 via-slate-900 to-slate-800 text-white font-['SF_Mono',monospace] overflow-x-hidden">
        ...
        <div className="relative z-10 p-3 sm:p-6 max-w-7xl mx-auto">
          ...
          {/* Data Management — central DB management, only on local node */}
          {!metrics._fleet_node && (
            <DataManagementCard backendUrl={backendUrlRef.current} authHeaders={authHeaderRef.current} />
          )}
          ...
          <SystemMetricsHistoryCard backendUrl={backendUrlRef.current} authHeaders={authHeaderRef.current} />
...
const DataManagementCard = ({ backendUrl, authHeaders }) => {
  const [open, setOpen] = React.useState(false);
  const [info, setInfo] = React.useState({ earnings: null, sessions: null, traffic: null, uptime: null });
  const [status, setStatus] = React.useState({}); // {key: 'deleting…'|'✓ …'|'✗ …'}
  const [pending, setPending] = React.useState({}); // {key: days}

  const load = React.useCallback(() => {
    if (!backendUrl) return;
    const h = authHeaders || {};
    Promise.all([
      fetch(`${backendUrl}/earnings/snapshots/info`, { headers: h }).then(r => r.json()).catch(() => null),
      fetch(`${backendUrl}/sessions/db/stats`, { headers: h }).then(r => r.json()).catch(() => null),
      fetch(`${backendUrl}/traffic/info`, { headers: h }).then(r => r.json()).catch(() => null),
    ]).then(([e, s, t]) => setInfo({ earnings: e, sessions: s, traffic: t }));
  }, [backendUrl]);

  const handleToggle = () => { if (!open) load(); setOpen(v => !v); setPending({}); setStatus({}); };

  const doDelete = async (key, endpoint, body) => {
    const pk = `${key}_${JSON.stringify(body)}`;
    if (pending[pk] !== true) { setPending(p => ({...p, [pk]: true})); return; }
    setPending(p => { const n={...p}; delete n[pk]; return n; });
    setStatus(s => ({...s, [key]: 'deleting…'}));
    try {
      ...
      const d = await r.json();
      setStatus(s => ({...s, [key]: d.error ? `✗ ${d.error}` : `✓ ${d.message}`}));
      load();
    } catch(e) { setStatus(s => ({...s, [key]: `✗ ${e.message}`})); }
    setTimeout(() => setStatus(s => { const n={...s}; delete n[key]; return n; }), 7000);
  };

  const doUptimeReset = async () => {
    if (!window.confirm('Reset local uptime tracking? 24h/30d local counters will restart from zero.')) return;
    try {
      const r = await fetch(`${backendUrl}/uptime/reset`, { method: 'POST', headers: authHeaders || {} });
      const d = await r.json();
      setStatus(s => ({...s, uptime: `✓ ${d.message}`}));
    } catch(e) { setStatus(s => ({...s, uptime: `✗ ${e.message}`})); }
    setTimeout(() => setStatus(s => { const n={...s}; delete n.uptime; return n; }), 7000);
  };

  const KeepButtons = ({ dbKey, endpoint, options }) => (
    <div className="flex flex-wrap gap-1.5 mt-1.5">
      {options.map(({ days, label }) => {
        const pk = `${dbKey}_${JSON.stringify(days === 0 ? {period:'all'} : {keep_last_days: days})}`;
        const isPending = pending[pk] === true;
        return (
          <button key={days}
            onClick={() => doDelete(dbKey, endpoint, days === 0 ? {period:'all'} : {keep_last_days: days})}
            disabled={status[dbKey] === 'deleting…'}
            className={`px-2 py-1 text-[10px] rounded border transition disabled:opacity-30 ${
              isPending
                ? 'bg-amber-500/20 text-amber-300 border-amber-500/40 animate-pulse'
                : days === 0
                  ? 'bg-slate-800/60 text-slate-500 border-slate-700 hover:border-red-500/40 hover:text-red-300'
                  : 'bg-slate-800/60 text-slate-500 border-slate-700 hover:border-emerald-500/30 hover:text-slate-200'
            }`}>
            {isPending ? '⚠ Confirm?' : label}
          </button>
        );
      })}
    </div>
  ...
  const sections = [
    {
      key: 'earnings',
      label: 'Earnings Snapshots',
      sub: 'earnings_history.db · daily/weekly/monthly tracking',
      endpoint: 'earnings/snapshots/delete',
      stat: info.earnings ? `${info.earnings.total ?? '?'} snapshots · ${info.earnings.oldest ?? '—'} → ${info.earnings.newest ?? '—'}` : '…',
      options: [{days:7,label:'Keep 7d'},{days:14,label:'Keep 14d'},{days:30,label:'Keep 30d'},{days:90,label:'Keep 90d'},{days:180,label:'Keep 180d'},{days:0,label:'Delete all'}],
    },
    {
      key: 'sessions',
      label: 'Session Archive',
      sub: 'sessions_history.db · full session history with frozen tokens',
      endpoint: 'sessions/delete',
      stat: info.sessions ? `${info.sessions.total ?? '?'} sessions · ${(info.sessions.oldest ?? '—').slice(0,10)} → ${(info.sessions.newest ?? '—').slice(0,10)}` : '…',
      options: [{days:30,label:'Keep 30d'},{days:90,label:'Keep 90d'},{days:180,label:'Keep 180d'},{days:365,label:'Keep 1y'},{days:0,label:'Delete all'}],
    },
    {
      key: 'traffic',
      label: 'Traffic History',
      sub: 'traffic_history.db · VPN + NIC daily totals from vnstat',
      endpoint: 'traffic/delete',
      stat: info.traffic ? `${info.traffic.total ?? '?'} days · ${info.traffic.oldest ?? '—'} → ${info.traffic.newest ?? '—'}` : '…',
      options: [{days:30,label:'Keep 30d'},{days:90,label:'Keep 90d'},{days:180,label:'Keep 180d'},{days:0,label:'Delete all'}],
    },
  ];
  ...
  return (
    <div className="mb-6 p-4 bg-slate-800/30 border border-slate-700 rounded-lg backdrop-blur">
      <div className="flex items-center justify-between">
        <div>
          <h3 className="text-sm font-semibold text-slate-300 tracking-wide">Data Management</h3>
          <p className="text-[10px] text-slate-600 mt-0.5">Manage what the toolkit keeps in its local databases</p>
        </div>
        <button onClick={handleToggle}
          className={`px-3 py-1.5 text-xs rounded border font-medium transition ${open
      ...
      {open && (
        <div className="mt-4 space-y-4">
          {sections.map(({ key, label, sub, endpoint, stat, options }) => (
            <div key={key} className="p-3 border border-slate-700/50 rounded-lg bg-[var(--panel-bg,#0a0a0a)]">
              <div className="flex items-start justify-between gap-2 mb-1">
                <div>
                  <div className="text-xs font-semibold text-slate-300">{label}</div>
                  <div className="text-[10px] text-slate-600 mt-0.5">{sub}</div>
                </div>
                <div className="text-[10px] text-slate-500 text-right shrink-0">{stat}</div>
              </div>
              <p className="text-[10px] text-slate-700 mb-1">Keep last → deletes everything older than selected period</p>
              <KeepButtons dbKey={key} endpoint={endpoint} options={options} />
              {Object.keys(pending).some(k => k.startsWith(key)) && (
                <div className="mt-1 text-[10px] text-amber-400">
                  Click again to confirm — cannot be undone.
                  <button onClick={() => setPending(p => { const n={...p}; Object.keys(n).filter(k=>k.startsWith(key)).forEach(k=>delete n[k]); return n; })} className="ml-2 text-slate-500 hover:text-slate-300">Cancel</button>
                </div>
              )}
              {status[key] && (
                <div className={`mt-1.5 px-2 py-1 rounded text-[10px] font-semibold ${status[key].startsWith('✓') ? 'bg-emerald-500/10 text-emerald-300' : status[key] === 'deleting…' ? 'text-slate-500' : 'bg-red-500/10 text-red-300'}`}>
                  {status[key]}
                </div>
              )}
            </div>
          ...
          {/* Uptime tracking reset */}
          <div className="p-3 border border-slate-700/50 rounded-lg bg-[var(--panel-bg,#0a0a0a)]">
            <div className="flex items-center justify-between">
              <div>
                <div className="text-xs font-semibold text-slate-300">Local Uptime Tracking</div>
                <div className="text-[10px] text-slate-600 mt-0.5">uptime_log.json · 24h/30d local ping history</div>
              </div>
              <button onClick={doUptimeReset}
                className="px-2 py-1 text-[10px] rounded border border-slate-700 text-slate-500 hover:border-amber-500/40 hover:text-amber-300 transition">
                ↺ Reset tracking
              </button>
            </div>
            {status.uptime && (
              <div className={`mt-1.5 px-2 py-1 rounded text-[10px] font-semibold ${status.uptime.startsWith('✓') ? 'bg-emerald-500/10 text-emerald-300' : 'bg-red-500/10 text-red-300'}`}>
                {status.uptime}
              </div>
            )}
          </div>
