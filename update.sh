...
# Disable use_pty for this user so sudo works from systemd timers and non-interactive shells
Defaults:$_REAL_USER !use_pty
$_REAL_USER ALL=(ALL) NOPASSWD: $TOOLKIT_DIR/update.sh, /sbin/sysctl, /usr/sbin/sysctl, /usr/sbin/ethtool, /usr/sbin/conntrack, /usr/local/bin/mysterium-rps-watcher.sh, /usr/local/bin/mysterium-rps-setup.sh, /usr/bin/tee /etc/sysctl.d/*, /usr/bin/tee /etc/modules-load.d/*, /usr/bin/tee /sys/module/nf_conntrack/parameters/hashsize, /usr/bin/tee /usr/local/bin/*, /usr/bin/tee /etc/systemd/system/mysterium-*.service, /usr/bin/tee /etc/systemd/system/mysterium-*.timer, /usr/bin/tee /etc/mysterium-node/config.toml, /usr/bin/tee /etc/mysterium-node/config-mainnet.toml, /usr/bin/chmod +x /usr/local/bin/mysterium-*, /bin/systemctl start mysterium-*, /bin/systemctl stop mysterium-*, /bin/systemctl restart mysterium-*, /bin/systemctl enable mysterium-*, /bin/systemctl disable mysterium-*, /bin/systemctl daemon-reload, /bin/systemctl reset-failed mysterium-toolkit, /usr/bin/systemctl start mysterium-*, /usr/bin/systemctl stop mysterium-*, /usr/bin/systemctl restart mysterium-*, /usr/bin/systemctl enable mysterium-*, /usr/bin/systemctl disable mysterium-*, /usr/bin/systemctl daemon-reload, /usr/bin/systemctl reset-failed mysterium-toolkit, /usr/sbin/ufw, /usr/sbin/iptables, /sbin/iptables, /usr/sbin/iptables-legacy, /sbin/iptables-legacy, /usr/sbin/iptables-nft, /usr/sbin/ip6tables, /sbin/ip6tables, /usr/sbin/nft, /sbin/nft, /usr/bin/tee /sys/devices/system/cpu/*/cpufreq/scaling_governor, /usr/bin/cpupower, /usr/bin/fail2ban-client, /usr/local/bin/fail2ban-client, /bin/fail2ban-client, /usr/bin/tee /etc/fail2ban/jail.local, /usr/bin/tee /etc/fail2ban/filter.d/*, /usr/bin/tee /etc/sudoers.d/mysterium-toolkit, /usr/bin/chmod 440 /etc/sudoers.d/mysterium-toolkit, /usr/sbin/visudo -c -f /etc/sudoers.d/mysterium-toolkit, /usr/bin/rm -f /etc/sudoers.d/mysterium-toolkit"
_SUDOERS_CURRENT=""
