...
Data from one node showing up under another node's name, in three separate places, plus two blocks of dead code that made a subsystem lie about what it recommends.
...
- feat (earnings snapshots record which node they came from): a `provider_id` column is added to existing databases on startup. Existing rows keep an empty value because it cannot be reconstructed. The primary key is still `time` alone, so two nodes writing at the same timestamp still collide with `INSERT OR IGNORE` dropping the second — a table rebuild with `UNIQUE(time, provider_id)` is the real fix and is deliberately not in this release
