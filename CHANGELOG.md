...
- feat (the node's self-updater can be turned off from the dashboard): node 1.39.x installs myst-updater.timer, which changes the node version on its own six-hourly schedule. Whether that is wanted depends on the machine — a fleet master restarting mid-session is not the same as a spare node. The badge shows the state and toggles it, writing MYST_UPDATER_ENABLED in `/etc/default/myst-updater` through the same sudo tee route used for the fail2ban files, with a matching sudoers entry. Both switches are read, since an active timer with the flag off does nothing at all
