...
![Version](https://img.shields.io/badge/version-1.3.10-brightgreen) ![License](https://img.shields.io/badge/license-AGPL--3.0-blue) ![Platform](https://img.shields.io/badge/platform-Linux-lightgrey) ![Python](https://img.shields.io/badge/python-3.8%2B-blue)
...
### Step 12a — Re-apply systemd on re-run

If the toolkit directory was moved since a previous install, this re-writes the systemd unit's `WorkingDirectory` so the service still starts correctly. Silent no-op on a first-time install.

---

### Step 12.5 — Fail2ban (optional)

Offered if `fail2ban` is installed or installable. Sets up the `mysterium-dashboard` jail in the toolkit-managed block of `/etc/fail2ban/jail.local`. Skip to configure it later from the dashboard's Security card.

---

### Step 12.6 — Tailscale (optional)

Offered if Tailscale is detected or you want to install it — useful for reaching a fleet node's dashboard over its Tailscale IP instead of a public one. Skip if you already have your own remote-access setup.
...
Every session is saved to SQLite with token values frozen the moment the session ends — before Mysterium zeroes them after settlement. The archive survives node restarts, re-installs, and settlements. View full history, filter by country, service type, or date. A **search box** finds every session for any consumer wallet (`0x…`) or session ID across the entire archive, and every session ID is **click-to-copy** for sharing with Mysterium support. **Export** the archive (or a single wallet's sessions) to **CSV or TXT** for the last 30/90 days or all history — generated from the frozen archive, so settled earnings stay accurate.
...
- **Payment config tuner** — read and write all node-consumed payment configuration keys live via `myst config set`, without editing config files manually. Every key below is verified against the Mysterium node source for the running version — no key is shown that the node doesn't actually read:

| Key | Default | Description |
|-----|---------|-------------|
| `payments.zero-stake-unsettled-amount` | 5.0 MYST | Auto-settle threshold — node starts trying to settle once unsettled MYST reaches this |
| `payments.unsettled.max-amount` | 20.0 MYST | Hard ceiling — above this the node always tries to settle, regardless of fees |
| `payments.settle.max-fee-percentage` | 0.05 (5%) | Below the ceiling, only settle once the blockchain tx fee is under this fraction of the amount — a gas-efficiency check, separate from Hermes's cut |
| `payments.provider.invoice-frequency` | 60s | How often to send payment invoices during a session |

**Hermes always takes a fixed 20% cut at settlement** — a 12.5 MYST threshold arrives as ~10 MYST. This is constant and none of the keys above change it; `max-fee-percentage` only affects *when* the node settles, not *how much* you receive.

Two built-in presets: **Standard · Stable Node** and **High Load · 50+ Sessions** (raises thresholds and invoice interval to reduce rate limiting on nodes with 50+ concurrent sessions).
...
Full control over all 7 persistent databases from the dashboard. Delete by type, by date range, or clear all. Two-click confirmation on all destructive actions. All history is kept indefinitely by default — automatic daily pruning only activates for a data type once you explicitly save a retention window for it here.
