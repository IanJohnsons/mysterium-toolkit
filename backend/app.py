...
# ── TLS (v1.4.0) ─────────────────────────────────────────────────────────────
# Off by default so existing installs are unaffected by an update. When enabled,
# the dashboard and the fleet peer endpoints are served over HTTPS using a
# self-signed certificate generated by setup.sh. No domain, no Let's Encrypt and
# no reverse proxy are required — the fleet master pins the certificate instead
# of validating it against a certificate authority.
HTTPS_ENABLED: bool = bool(setup_config.get('https_enabled', False))
TLS_CERT: str = setup_config.get('tls_cert', 'config/tls/cert.pem')
TLS_KEY: str = setup_config.get('tls_key', 'config/tls/key.pem')

# Thread pool for the production WSGI server. 30 threads with a 200-deep accept
# queue handled 150 concurrent requests at a 13 ms median in testing, against
# 80 ms for the Flask development server. Pi mode uses fewer threads because each
# one costs stack memory and a Pi will never see that kind of concurrency.
CHEROOT_THREADS: int = int(setup_config.get('server_threads', 10 if PI_MODE else 30))
CHEROOT_QUEUE: int = int(setup_config.get('server_queue_size', 200))
if PI_MODE:
        ...
        if p.exists():
            try:
                ...
                for i, n in enumerate(nodes):
                    ...
                    # TLS fields (v1.4.0) — only meaningful when toolkit_url is https://
                    if n.get('tls_cert'):
                        entry['tls_cert'] = n['tls_cert']
                    if 'tls_verify' in n:
                        entry['tls_verify'] = bool(n['tls_verify'])
                    result.append(entry)
                if result:
                    _nodes_json_path = p
...
def _peer_verify(node_entry):
    """Return the value for requests' verify= parameter for this peer (v1.4.0).

    http://           -> True  (no TLS in play, value is irrelevant)
    tls_verify: false -> False (encrypted but NOT authenticated — see warning)
    tls_cert set      -> path to the pinned certificate
    otherwise         -> True  (normal CA validation, for real certificates)

    Pinning a self-signed certificate is stronger than CA validation: the
    connection is bound to that one certificate rather than to any certificate
    a public authority happens to sign.
    """
    url = node_entry.get('toolkit_url', '')
    if not url.startswith('https://'):
        return True

    node_id = node_entry.get('id', '?')

    if node_entry.get('tls_verify') is False:
        if node_id not in _tls_warned:
            logger.warning(
                f"Node {node_id}: tls_verify is false — traffic is encrypted but the "
                f"certificate is not checked, so a man-in-the-middle attack is possible. "
                f"Use only on a network you trust."
            )
            _tls_warned.add(node_id)
        try:
            import urllib3
    ...
    cert = node_entry.get('tls_cert')
    if cert:
        if Path(cert).exists():
            return str(cert)
        if node_id not in _tls_warned:
            logger.warning(
                f"Node {node_id}: tls_cert '{cert}' not found — falling back to "
                f"certificate authority validation, which will fail for a self-signed "
                f"certificate. Copy the peer's cert.pem to that path."
            )
            _tls_warned.add(node_id)
...
def _get_node_heavy_data(node_id, toolkit_url, headers, verify=True):
    ...
    if toolkit_url:
        try:
            ...
            if resp.status_code == 200:
                ...
                heavy = _get_node_heavy_data(node_id, toolkit_url, headers, verify)
                result.update({
                    'peer_mode':        True,
                    'status':           ns.get('status', 'unknown'),
                    'uptime':           ns.get('uptime', '0s'),
...
def _current_branch():
    """Return the git branch this install is on, defaulting to 'main' (v1.4.0)."""
    cached = getattr(_current_branch, '_cache', None)
    if cached:
        return cached
    branch = 'main'
    try:
        ...
        # Detached HEAD reports 'HEAD'; fall back to main in that case.
        if out.returncode == 0 and name and name != 'HEAD':
            branch = name
    except Exception:
        pass
    _current_branch._cache = branch
    return branch
...
if __name__ == '__main__':
    ...
    ✓ Auth:     {auth_mode}
    {'✓ Frontend: ' + ('https' if HTTPS_ENABLED else 'http') + '://0.0.0.0:' + str(PORT) + '/' if _has_dist else '⚠ Frontend: start Vite with ./start.sh'}
    """)
