...
# ── TLS certificate generation (v1.4.4) ──────────────────────────────────────
...
TLS_PY_EOF
    ...
    if [ -f "$_TLS_CERT" ] && [ -f "$_TLS_KEY" ]; then
        ...
        # must not be reported as success — the certificate alone does nothing.
        _PY="$TOOLKIT_DIR/venv/bin/python3"
...
import json, pathlib
try:
    print(json.loads(pathlib.Path('config/setup.json').read_text()).get('https_enabled'))
except Exception:
    print('False')
" 2>/dev/null)
        if [ "$_TLS_ON" != "True" ]; then
            echo -e "  ${RED}✗ Certificate created, but config/setup.json could not be updated${NC}"
            echo -e "  ${DIM}    TLS is NOT active. Add this to config/setup.json by hand:${NC}"
            echo -e "  ${DIM}      \"https_enabled\": true,${NC}"
            echo -e "  ${DIM}      \"tls_cert\": \"config/tls/cert.pem\",${NC}"
            echo -e "  ${DIM}      \"tls_key\": \"config/tls/key.pem\"${NC}"
            return 1
        fi
                ...
                cat > "$NODES_JSON" << 'NODES_EOF'
{
  "nodes": [
    ...
    {
      "id": "node2",
      "label": "My Second Node — example with TLS",
      "url": "http://REPLACE_WITH_NODE_IP:4050",
      "toolkit_url": "https://REPLACE_WITH_NODE_IP:5000",
      "toolkit_api_key": "REPLACE_WITH_DASHBOARD_API_KEY",
...
NODES_EOF
                ...
                echo -e "  ${DIM}  TLS: node2 shows how to reach a node over https. Copy that node's${NC}"
                echo -e "  ${DIM}  config/tls/cert.pem to this machine and point tls_cert at it.${NC}"
                echo -e "  ${DIM}  Plain http nodes need no tls fields — a mixed fleet is fine.${NC}"
                ;;
        esac
    fi
    echo
